...
class Array_lock_optimized
{
    ...
    int numthreads;
    static int Slot;
    #pragma omp threadprivate(Slot)

    public:
    Array_lock_optimized(int n) : flag(new volatile bool[n]) 
    {    
        for (int i = 0; i < n; ++i)
            flag[i] = false;
        flag[0] = true;
    ...
    void lock() {
        Slot = tail.fetch_add(1)%numthreads;
        while (!flag[Slot])
        {}
    }
    void unlock() {
...
///////////////////////////////////////////////////////////// Array Lock optimized

void run_Array_lock_optimized(int numthreads, int iterations, int numofiter) 
{    
    double runtime = 0;
    double totruntime = 0;
    double HowFair = 0;
    ...
    int len = 0;
    double turns[(numthreads-1)*8+1];     // keeps count of how often a thread got the CS, long has 8 bytes, so write one value every 8 slots to avoid false sharing
    Array_lock_optimized mylock(numthreads);
    ...
    Timevar = Varianz(Time,numofiter);
    output("Array_lock_optimized",iterations,numthreads,numofiter,totruntime,HowFair,Timevar);
}
...
int main(int argc, char *argv[]) 
{
    ...
    {   
        ...
        for (int i = 1; i < numthreadsmax+1; i++)
        {
            ...
            if (mode == 10) {
                Locker.run_Array_lock_optimized(numthreads, iterations, numofiter);
            }
        }
        std::cout << std::endl;
    }
